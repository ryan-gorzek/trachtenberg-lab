...
obj.ctx.rd3 <- obj.i[, obj.i$class_nn %in% c("glutamatergic", "gabaergic", "nonneuronal")]
...
obj.i.rd2 <- LabelByNearestNeighbors(obj.i.rd2, "subclass", fraction = 0.25, n.neighbors = 100)
...
obj.i.rd3 <- IntegrateObjects(obj, obj.ctx.rd3, resolutions = c(0.5), subsample = FALSE)
obj.i.rd3 <- LabelByNearestNeighbors(obj.i.rd3, "subclass", fraction = 0.25, n.neighbors = 100)
...
DimPlot(obj.i.rd3, group.by = "subclass_nn", label = TRUE) + NoLegend() + xlim(-12, 12) + ylim(-12, 12) + coord_equal()
...
obj.ctx.rdf <- obj.ctx.all[, obj.ctx.all$subclass_nn == "None"]
obj.i.rdf <- IntegrateObjects(obj, obj.ctx.rdf, resolutions = c(0.5), subsample = FALSE)
obj.i.rdf <- LabelByNearestNeighbors(obj.i.rdf, "subclass", fraction = 0.25, n.neighbors = 100)
...
DimPlot(obj.i.rdf, group.by = "subclass_nn", label = TRUE) + NoLegend() + xlim(-12, 12) + ylim(-12, 12) + coord_equal()
...
obj.ctx.all <- readRDS("E:/STOmics/seurat/Mouse/mouse_stereoseq_ctx_COLS_nofilt.rds")
...
obj.ctx.all$subclass_nn[cell_ids] <- assigned_subclasses
# Round 3
assigned_subclasses <- obj.i.rd3$subclass_nn[!is.na(obj.i.rd3$subclass_nn)]
cell_ids <- names(assigned_subclasses)
obj.ctx.all$subclass_nn[cell_ids] <- assigned_subclasses
# Final Round
assigned_subclasses <- obj.i.rdf$subclass_nn[!is.na(obj.i.rdf$subclass_nn)]
cell_ids <- names(assigned_subclasses)
obj.ctx.all$subclass_nn[cell_ids] <- assigned_subclasses
...
saveRDS(obj.ctx.all, "E:/obj_ctx_all_filt_Mouse.rds")
