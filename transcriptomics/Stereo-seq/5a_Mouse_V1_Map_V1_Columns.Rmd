...
obj.glutamatergic <- ClusterSCT(obj.glutamatergic, 0.2)
...
saveRDS(obj.i, "E:/STOmics/seurat/Mouse/mouse_stereoseq_ctx_COLs_EXT_integrated.rds")
...
obj.i <- readRDS("E:/STOmics/seurat/Mouse/mouse_stereoseq_ctx_COLs_EXT_integrated.rds")
...
obj.i <- LabelByNearestNeighbors(obj.i, "type", fraction = 0.25, n.neighbors = 100)
...
obj.glutamatergic.l5it <- obj.glutamatergic[, obj.glutamatergic$subclass %in% c("L2/3", "L4", "L5IT", "L6IT", "L5NP", "L5PT", "L6CT", "L6b")]
Idents(obj.glutamatergic.l5it) <- "subclass"
degs <- FindMarkers(obj.glutamatergic.l5it, ident.1 = "L5IT")
...
obj.ctx.glutamatergic.L23 <- obj.ctx[, obj.ctx$type_nn %in% c("L2/3_A", "L2/3_B", "L2/3_C")]
Idents(obj.ctx.glutamatergic.L23) <- "type_nn"
ImageDimPlot(obj.ctx.glutamatergic.L23, fov = "COL", size = 3, axes = TRUE)
...
coords <- GetTissueCoordinates(obj.ctx.glutamatergic.L23)
df <- data.frame(
  y = coords$x,
  cluster = obj.ctx.glutamatergic.L23$type_nn
)

# 3. Filter and set subclass as a factor to enforce order
df <- df %>%
  filter(!is.na(cluster) & cluster != "None") %>%
  mutate(cluster = factor(cluster, levels = c("L2/3_A", "L2/3_B", "L2/3_C")))
...
ggsave("E:/Opossum_Paper/Figure S3/Mouse_L23_Density_type.svg", plot = p)
...
ImageDimPlot(obj.ctx.nonneuronal, fov = "COL", size = 3, cols = colors[1:6], axes = TRUE)
...
saveRDS(obj.ctx.glutamatergic, "E:/STOmics/seurat/Mouse/mouse_stereoseq_ctx_glutamatergic_COLs_integrated.rds")
saveRDS(obj.ctx.gabaergic, "E:/STOmics/seurat/Mouse/mouse_stereoseq_ctx_gabaergic_COLs_integrated.rds")
saveRDS(obj.ctx.nonneuronal, "E:/STOmics/seurat/Mouse/mouse_stereoseq_ctx_nonneuronal_COLs_integrated.rds")
