...
% Load data from CSV
data = readtable('E:/Transcriptomics_V1/Integration/PCs/mouse_pc_data_subsample_with_subclass.csv');
...
ylabel('PC2');
zlabel('PC3');
title('2D Scatter Plot of Principal Components');
grid on;
...
% Load data from CSV
data = readtable('E:/Transcriptomics_V1/Integration/PCs/opossum_pc_data_subsample_with_subclass.csv');
...
% Load data from CSV
data = readtable('E:/Transcriptomics_V1/Integration/PCs/macaque_pc_data_with_subclass.csv');

% Extract the first three PCs
pc1 = data.PC_1;  % Assuming first column is PC1
pc2 = data.PC_2 * -1;  % Assuming second column is PC2
pc3 = data.PC_3;  % Assuming third column is PC3
...
for i = 1:length(subclass_groups)
    idx = (subclass_idx == i);
    scatter3(pc1(idx), pc2(idx), pc3(idx), 15, 'filled', 'DisplayName', subclass_groups{i}); % , cmap(i, :)
end
...
% Extract the first three PCs
pc1 = data.PC_1;  % Assuming first column is PC1
pc2 = data.PC_2;  % Assuming second column is PC2
pc3 = data.PC_3;  % Assuming third column is PC3
...
% Define a colormap based on the number of unique subclasses
hex_colors = {'#C692B8'; '#AAAAAA'}; % Example hex colors
...
%% Shared (Opossum / Macaque)

% Load data from CSV
data = readtable('E:/opossum_macaque_shared_pc_data_subsample_with_subclass.csv');

% Extract the first three PCs
pc1 = data.pca_1;  % Assuming first column is PC1
pc2 = data.pca_2 * -1;  % Assuming second column is PC2
...
for i = 1:length(species_groups)
    idx = (subclass_idx == i);
    scatter3(pc1(idx), pc2(idx), pc3(idx), 5, cmap(i, :), 'filled', 'DisplayName', species_groups{i}, 'MarkerFaceAlpha', 0.5);
end
