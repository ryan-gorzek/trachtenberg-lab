...
axis equal;
axis square;
% view(95, 10); % left side
% view(12, 13); % back side
% view(-85, -15) % right side
hold off;
...
tetra_edges = [1 2; 1 3; 1 4; 2 3; 2 4; 3 4]; % Indices defining the edges of a tetrahedron
for k = 1:size(tetra_edges, 1)
    plot3((arcOrig(tetra_edges(k, :), 1) * -1) - 3, arcOrig(tetra_edges(k, :), 3)- 1, arcOrig(tetra_edges(k, :), 2) - 5, '-k', 'LineWidth', 1.5);
    % plot3((arcOrig(tetra_edges(k, :), 1) * -1), arcOrig(tetra_edges(k, :), 3), arcOrig(tetra_edges(k, :), 2), '-k', 'LineWidth', 1.5);
end

axis equal;
axis square;
% view(196, 18); % left side
% view(107, -2); % back side
view(9, 13); % right side
hold off;
