...
mapping.modality = list("Single_Nucleus" = mapping.data, "Stereo_Seq" = mapping.data)
objs.opossum$Glutamatergic <- readRDS("E:/Transcriptomics_V1/Opossum/seurat/opossum_v1_glutamatergic_processed.rds")
objs.opossum$GABAergic <- readRDS("E:/Transcriptomics_V1/Opossum/seurat/opossum_v1_gabaergic_processed.rds")
objs.opossum$Nonneuronal <- readRDS("E:/Transcriptomics_V1/Opossum/seurat/opossum_v1_nonneuronal_processed.rds")
obj.opossum <- merge(objs.opossum[[1]], y = objs.opossum[2:3])
obj.ctx <- readRDS("E:/STOmics/seurat/opossum_stereoseq_ctx_1_clustered.rds")
obj.ctx$subclass <- obj.ctx$SCT_snn_res.1

iteration.list = list("Single_Nucleus" = c(), "Stereo_Seq" = c())
iteration.list$Single_Nucleus <- SubsampleObjectMultipleIterations(obj.opossum, "subclass", 100, 10)
iteration.list$Stereo_Seq <- SubsampleObjectMultipleIterations(obj.ctx, "subclass", 100, 10)

for (it in 1:10) {

  objs <- list()
  objs[[1]] <- subset(obj.opossum, cells = as.character(iteration.list$Single_Nucleus[[it]]))
  objs[[2]] <- subset(obj.ctx, cells = as.character(iteration.list$Stereo_Seq[[it]]))
  objs.m <- MapObjects(objs[[1]], objs[[2]], c("subclass"), assay = "SCT")
  mapping.modality$Single_Nucleus[["cell.id"]] <- c(mapping.modality$Single_Nucleus[["cell.id"]], as.character(colnames(objs.m[[1]])))
  mapping.modality$Single_Nucleus[["subclass"]] <- c(mapping.modality$Single_Nucleus[["subclass"]], as.character(objs.m[[1]]$subclass))
  mapping.modality$Single_Nucleus[["predicted.subclass"]] <- c(mapping.modality$Single_Nucleus[["predicted.subclass"]], as.character(objs.m[[1]]$predicted.subclass))
  mapping.modality$Single_Nucleus[["predicted.subclass.score"]] <- c(mapping.modality$Single_Nucleus[["predicted.subclass.score"]], as.numeric(objs.m[[1]]$predicted.subclass.score))
  mapping.modality$Stereo_Seq[["cell.id"]] <- c(mapping.modality$Stereo_Seq[["cell.id"]], as.character(colnames(objs.m[[2]])))
  mapping.modality$Stereo_Seq[["subclass"]] <- c(mapping.modality$Stereo_Seq[["subclass"]], as.character(objs.m[[2]]$subclass))
  mapping.modality$Stereo_Seq[["predicted.subclass"]] <- c(mapping.modality$Stereo_Seq[["predicted.subclass"]], as.character(objs.m[[2]]$predicted.subclass))
  mapping.modality$Stereo_Seq[["predicted.subclass.score"]] <- c(mapping.modality$Stereo_Seq[["predicted.subclass.score"]], as.numeric(objs.m[[2]]$predicted.subclass.score))
...
idents <- c("IT_A", "IT_B", "IT_C", "IT_D", "L5NP", "L5PT", "L6CT", "L6b", "Pvalb", "Sst", "Vip", "Lamp5", "Frem1", "Stac", "Astro", "Micro", "OD", "OPC", "Endo", "VLMC")
ident.order <- c(c("IT_A", "IT_B", "IT_C", "IT_D", "L5NP", "L5PT", "L6CT", "L6b", "Pvalb", "Sst", "Vip", "Lamp5", "Frem1", "Stac", "Astro", "Micro", "OD", "OPC", "Endo", "VLMC"), as.character(unique(obj.ctx$subclass)))

PlotSubsampledMappedLabelsHeatmap(mapping.modality$Stereo_Seq$subclass, mapping.modality$Stereo_Seq$predicted.subclass, 
                                  idents, normalize = "row", 
                                  ident.order = ident.order) + scale_fill_gradient2(low = "white", mid = "blue", high = "red", midpoint = 50)
...
saveRDS(mapping.modality, "E:/Transcriptomics_V1/Integration/opossum_stereoseq_subsampled_mapping_10it.rds")
