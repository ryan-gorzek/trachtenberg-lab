...
obj.mouse.CGE <- subset(obj.mouse.gabaergic, idents = c("Vip", "Lamp5", "Frem1"))
...
subclass.markers.opossum <- FindAllMarkers(obj.opossum.CGE, min.pct = 0.1, only.pos = TRUE)
subclass.markers.mouse <- FindAllMarkers(obj.mouse.CGE, min.pct = 0.1, only.pos = TRUE)

cluster_pairs = list(c("Vip", "Vip"), c("Lamp5", "Lamp5"), c("Frem1", "Frem1"))

PlotSubclassGeneCountCDFDiff(list(subclass = subclass.markers.opossum), 
                             list(subclass = subclass.markers.mouse), 
                             cluster_pairs, as.character(colors_list[c("Vip", "Lamp5", "Frem1")]), 
...
subclass_pairs = list(list(c("Vip", "Lamp5"), c("Vip", "Lamp5")), 
                      list(c("Lamp5", "Frem1"), c("Lamp5", "Frem1")), 
                      list(c("Vip", "Frem1"), c("Vip", "Frem1")))
...
for (pair in subclass_pairs) {
  ...
  de_results_opossum[[paste(sbcl_1_1, sbcl_1_2, sep = "_vs_")]] <- FindMarkers(obj.opossum.CGE, ident.1 = sbcl_1_1, ident.2 = sbcl_1_2, test.use = "wilcox")
  de_results_mouse[[paste(sbcl_2_1, sbcl_2_2, sep = "_vs_")]] <- FindMarkers(obj.mouse.CGE, ident.1 = sbcl_2_1, ident.2 = sbcl_2_2, test.use = "wilcox")
...
p <- PlotPairwiseSubclassGeneCountCDFDiff(de_results_opossum, de_results_mouse, 
                                      subclass_pairs, 
                                      as.character(colors_list[c("Vip", "Lamp5", "Frem1")]), 
                                      min.pct = 0.25, 
                                      max.pval = 0.05) + ylim(-100, 100)

print(p)
ggsave("E:/Opossum_Paper/Figure 2/CGE_DE_Difference.svg", plot = p)
