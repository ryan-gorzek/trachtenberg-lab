---
title: "Integrate GABAergic Mouse Cat"
...
obj.cat <- LoadH5Seurat("E:/Cat_M1/seurat/cat_m1_gabaergic.h5seurat")
DefaultAssay(obj.cat) <- "RNA"
obj.cat$dataset <- "Cat_M1"
...
common.features <- intersect(rownames(obj.mouse), rownames(obj.cat))
obj.combined <- merge(obj.mouse[common.features,], y = obj.cat[common.features,])
...
obj.list[["Cat_M1"]] <- SCTransform(obj.list[["Cat_M1"]], vst.flavor = "v2", verbose = FALSE) %>%
               RunPCA(npcs = 30, verbose = FALSE) %>%
               RunUMAP(reduction = "pca", dims = 1:30, verbose = FALSE) %>%
               FindNeighbors(reduction = "pca", dims = 1:30, verbose = FALSE) %>%
               FindClusters(resolution = 1, algorithm = 4, method = "igraph")

DimPlot(obj.list[["Cat_M1"]], label = TRUE) + NoLegend() + xlim(-15, 15) + ylim(-15, 15) + coord_equal()
DimPlot(obj.list[["Cat_M1"]], label = TRUE, group.by = "subclass") + NoLegend() + xlim(-15, 15) + ylim(-15, 15) + coord_equal()
