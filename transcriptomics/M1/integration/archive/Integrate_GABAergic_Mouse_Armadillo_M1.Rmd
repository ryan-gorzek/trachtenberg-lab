...
obj.armadillo <- LoadH5Seurat("E:/Armadillo_M1/seurat/armadillo_m1_gabaergic_sct.h5seurat")
obj.armadillo$dataset <- "Armadillo_M1"
...
common.features <- intersect(rownames(obj.mouse), rownames(obj.armadillo))
obj.combined <- merge(obj.mouse[common.features,], y = obj.armadillo[common.features,])
...
obj.list[["Armadillo_M1"]] <- SCTransform(obj.list[["Armadillo_M1"]], vst.flavor = "v2", verbose = FALSE) %>%
               RunPCA(npcs = 30, verbose = FALSE) %>%
               RunUMAP(reduction = "pca", dims = 1:30, verbose = FALSE) %>%
               FindNeighbors(reduction = "pca", dims = 1:30, verbose = FALSE) %>%
               FindClusters(resolution = 1, algorithm = 4, method = "igraph")

DimPlot(obj.list[["Armadillo_M1"]], label = TRUE) + NoLegend() + xlim(-15, 15) + ylim(-15, 15) + coord_equal()
DimPlot(obj.list[["Armadillo_M1"]], label = TRUE, group.by = "subclass") + NoLegend() + xlim(-15, 15) + ylim(-15, 15) + coord_equal()
...
DimPlot(obj.combined.sct, reduction = "umap", group.by = "RNA_snn_res.0.5", raster = FALSE, label = TRUE) + xlim(-18, 18) + ylim(-18, 18) + coord_equal()
DimPlot(obj.combined.sct, reduction = "umap", group.by = "ar_integrated_snn_res.1", raster = FALSE, label = TRUE) + xlim(-18, 18) + ylim(-18, 18) + coord_equal()
...
SaveH5Seurat(obj.combined.sct, "E:/Armadillo_M1/seurat/mouse_armadillo_m1_gabaergic_integrated.h5seurat", overwrite = TRUE, verbose = TRUE)
