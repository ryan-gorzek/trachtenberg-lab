...
path = "E:/Imaging/";
stimdict = dictionary(["20", "30", "40", "50", "60"], ["randorisf", "battery1", "battery2", "battery3", "battery4"]);
subjects = ["pv_vip_03"];
...
for subj = subjects
    ...
    n_idx = endsWith(f, "000_000"); % ignore the concatenated session
    t_idx = endsWith(f, "010_000"); % ignore the trinoise session
    stimf = f(f_idx & ~n_idx & ~t_idx);
    ...
    for st = stimf
        ...
        % load calcium data
        [rtdata, stdata, xy, grid] = load_calcium_data(path, subj, sess, run);
...
keepvars = ["subject", ...
            "SNR_ROSF_spikes", "Ori_ROSF_spikes", "SF_ROSF_spikes", "F1F0_ROSF_spikes", "CV_ROSF_spikes", ... % , "eye_corr_ROSF_dFF", "quad_corr_ROSF_dFF", ...
...
%             "Dir_Cent_B4_dFF", "Dir_Surr_B4_dFF", "Ctrst_Cent_B4_dFF", "Ctrst_Surr_B4_dFF", ... % "eye_corr_B4_dFF", "quad_corr_B4_dFF", ...
...
% pooled
figure; tiledlayout(6, 3, "TileSpacing", "compact");
for feat = 1:nfeats
