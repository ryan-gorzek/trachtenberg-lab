...
%         kernPlot(plane_tbl, title=title, save_path=save_path);
    ...
    if genotypes(subj_ids == subj) == "KO"
        KO_tbl = vertcat(KO_tbl, subj_tbl);
    else
        Het_tbl = vertcat(Het_tbl, subj_tbl);
    end
end
...
% Draw colorbar for response linearity scatter point colors.
cbar = colorbar(f.Children(1), ...
                "AxisLocation", "out", ...
                "LineWidth", 1, ...
                "TickLabels", {}, ...
                "TickLength", 0, ...
                "Position", [0.715, 0.347, 0.01, 0.34]);
cbar.Label.String = "Response Linearity (F1/F0)";
cbar.Label.Position = [-1.3, 1, 0];
% Manually add tick labels for rotation.
colorbarTicks(f, cbar);
...
% Add mono/bino row labels and lines
annotation("textbox", ...
           "String", "Mono", ...
           "Interpreter", "none", ...
           "FontWeight","bold", ...
...
function ax_out = insetColorScatter(X, Y, cvals, cmap, fig, ax, NVAs)
% INSETPOLARSCATTER Add polar scatterplot to TiledLayout Chart.
arguments
    X
    Y
...
cmap1 = colormapData(dens1, [0, max(dens1)], colors{1}, nrows);
% X2, Y2
dens2 = ksdensity([X2, Y2], xi);
cmap2 = colormapData(dens2, [0, max(dens2)], colors{2}, nrows);
% disp([dens1.*((1/nrows)*(0.16/ncols)), dens2.*((1/nrows)*(0.16/ncols))]);
...
function colorbarTicks(fig, cbar)
...
f_temp = figure(100); ax_cbar = axes;

set(ax_cbar, "Color", "none", ...
             "XTick", [], ...
             "YAxisLocation", "right", ...
             "YTick", 0:0.5:2, ...
             ...
             "YLim", cbar.Limits);
copyobj(ax_cbar, fig);
fig.Children(1).Position = cbar.Position;
close(f_temp);
