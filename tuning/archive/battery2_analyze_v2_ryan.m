...
p2 = unique(param(:,2));
p2 = 60 ./ p2; % convert period in frames to Hz
nparam1 = length(p1);
nparam2 = length(p2);
...
[navg_mean_traces, navg_norm_traces] = tuning(rt, frame_on, frame_off, param, idx, [10, 0]);
% Get the stimulus response matrix for spikes
[navg_mean_spikes, navg_norm_spikes] = tuning(st, frame_on, frame_off, param, idx, [0, -25]);
