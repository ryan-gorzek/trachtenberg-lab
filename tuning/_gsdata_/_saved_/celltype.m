...
pyr_recs = ["pv_vip_01_6", "pv_vip_01_7"]; % , "pv_vip_04_1"
...
for sim = 1:10
    for pyr = pyr_recs
        for sst = sst_recs
            ...
            dset = ids(~nanidx, 1);
            all_idx = 1:size(X, 1);
            train_idx = ~ismember(dset, [pyr, sst]);
            test_idx_pyr = ismember(dset, pyr);
            test_idx_sst = ismember(dset, sst);
            if nnz(test_idx_pyr) > nnz(test_idx_sst)
                idx_pyr = find(test_idx_pyr);
                rand_idx_pyr = idx_pyr(randperm(numel(idx_pyr)));
                test_idx_pyr(rand_idx_pyr(1:nnz(test_idx_pyr) - nnz(test_idx_sst))) = 0;
            elseif nnz(test_idx_pyr) < nnz(test_idx_sst)
                idx_sst = find(test_idx_sst);
                rand_idx_sst = idx_sst(randperm(numel(idx_sst)));
                test_idx_sst(rand_idx_sst(1:nnz(test_idx_sst) - nnz(test_idx_pyr))) = 0;
            end
...
boxPlot(err, boxLabels={'Shuffle', 'Observed'}, boxColors={[1,1,1], [1,1,1]}, plotPoints=true, pointJitter="rand", pointSize=200);
ylim([0, 1]); ylabel("Cross-Validated Accuracy");
