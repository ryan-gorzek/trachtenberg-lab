...
% f.Position = [1278, 901, 679, 1336];
f.Position = [541, 1416, 1094, 461];
% title(t, NVAs.title, ...
%          "Interpreter", "none", ...
%          "FontWeight", "bold", ...
%          "FontSize", 15);
...
f_temp = figure(2); ax_polar = polaraxes("Parent", f_temp);
polarhistogram(data, ...
               -pi/32 : pi/16 : pi*63/32, ...
               "Normalization","probability", ...
               "FaceColor", NVAs.face_color, ...
               "EdgeColor", "none");
title(NVAs.title); axis tight;
set(ax_polar, ...
...
outer_position = ax.OuterPosition;
f_temp = figure(2); ax_polar = polaraxes("Parent", f_temp); hold on;
...
outer_position = ax.OuterPosition;
f_temp = figure(2); ax_scatter = axes; hold on; pbaspect([1,1,1]);
scatter(X, Y, ...
        20, ...
        cvals, ...
        "filled", ...
