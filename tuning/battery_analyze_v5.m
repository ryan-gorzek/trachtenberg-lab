...
for s = sid
    ...
    [expst.(stims(s)).spikes, expst.(stims(s)).dFF] = expstruct(stims(s), rtdata, stdata, ops, stat, frame_on, frame_off, params, stim_center, pix_per_deg, eye, quad);
...
for c = 1:numel(cells)
    if (on_dist(c) && on_snr(c)), scatter(ON(c, 1), ON(c, 2), "+r", "MarkerEdgeAlpha", 0.5); end % && SNR(c)
    if (off_dist(c) && off_snr(c)), scatter(OFF(c, 1), OFF(c, 2), "+b", "MarkerEdgeAlpha", 0.5); end % && SNR(c)
end
...
ctxmod_psth(expst, id, "Center", sz);
nexttile(12, [2, 2]); hold on;
ctxmod_psth(expst, id, "Surround", sz);
nexttile(40, [2, 2]); hold on;
ctxmod_psth(expst, id, "Cross", sz);
nexttile(42, [2, 2]); hold on;
ctxmod_psth(expst, id, "Iso", sz);
% center/surround tuning curves at different sizes
...
function ctxmod_psth(expst, id, context, sz)
